...
public final class AllowlistEntrySpecifications {
    ...
    public static DeleteSpecification<AllowlistEntry> deleteByPubkey(XonlyPublicKey pubkey) {
        return (root, cq, cb) -> cb.equal(root.get("pubkey"), pubkey.value.toHex());
    }

    public static DeleteSpecification<AllowlistEntry> deleteByPubkey(ByteString pubkey) {
        return deleteByPubkey(new XonlyPublicKey(new ByteVector32(pubkey.toByteArray())));
    }
}
